...
void GResult(VkResult result, const char* what = "Runtime error from vulkan_util::checkResult!")
{
...
void VulkanApplication::InitWindow()
{
	...
	mpWindow = glfwCreateWindow(mWidth, mHeight, "Romil's Vulkan Renderer", nullptr, nullptr);

	glfwSetWindowUserPointer(mpWindow, this);

	mpInputManager->SetWindow(mpWindow);
}

void VulkanApplication::FrameBufferCallback(GLFWwindow* window, int width, int height)
{
...
void VulkanApplication::Run()
{
	...
	device = getDevice();
	utility = new VUtility(*this);
	graphics_command_pool = getGraphicsCommandPool();
	compute_command_pool = getComputeCommandPool();
	initialize();
	Loop();
}
...
void VulkanApplication::Loop()
{
	...
	while (!glfwWindowShouldClose(mpWindow))
	{
		...
		setCamera(mCamera.getViewMatrix(), mCamera.position);
		requestDraw(delta_time);
...
void VulkanApplication::CreateInstance()
{
	...
	instance = VulkanRaii<vk::Instance>{
...
void VulkanApplication::setupDebugCallback()
{
	...
	callback = VulkanRaii<vk::DebugReportCallbackEXT>{
		temp_callback,
...
void VulkanApplication::createWindowSurface()
{
	...
	auto result = glfwCreateWindowSurface(instance.get(), mpWindow, nullptr, &temp_surface);
...
void VulkanApplication::findQueueFamilyIndices()
{
...
void VulkanApplication::createLogicalDevice()
{
	...
	graphics_device = VulkanRaii<vk::Device>{
		temp_device,
		[](vk::Device& device)
		{
			device.destroy();
...
void VulkanApplication::createCommandPools()
{
	...
	// graphics_queue_command_pool
	{
		...
		graphics_queue_command_pool = VulkanRaii<vk::CommandPool>(
	...
	// compute_queue_command_pool
	{
		...
		compute_queue_command_pool = VulkanRaii<vk::CommandPool>(
...
bool VulkanApplication::isDeviceSuitable(VkPhysicalDevice device, VkSurfaceKHR window_surface)
{
...
void VulkanApplication::resize(int width, int height)
{
	if (width == 0 || height == 0) return;

	std::tie(window_framebuffer_width, window_framebuffer_height) = getWindowFrameBufferSize();

	recreateSwapChain();
}

void VulkanApplication::requestDraw(float deltatime)
{
...
void VulkanApplication::setCamera(const glm::mat4& view, const glm::vec3 campos)
{
	view_matrix = view;
	this->cam_pos = campos;
}
...
{
	auto support_details = SwapChainSupportDetails::querySwapChainSupport(physical_device, getWindowSurface());
	...
	create_info.surface = getWindowSurface();
	...
	QueueFamilyIndices indices = QueueFamilyIndices::findQueueFamilies(physical_device, getWindowSurface());
	...
	GResult(vkCreateSwapchainKHR(graphicsdevice, &create_info, nullptr, &tmp_swap_chain), "Failed to create swap chain!");

	swap_chain = VulkanRaii<vk::SwapchainKHR>(
		tmp_swap_chain,
		[device = this->device](auto& obj){device.destroySwapchainKHR(obj); }
	);

	uint32_t image_count;
	vkGetSwapchainImagesKHR(graphicsdevice, swap_chain.get(), &image_count, nullptr);
	swap_chain_images.resize(image_count);
	vkGetSwapchainImagesKHR(graphicsdevice, swap_chain.get(), &image_count, swap_chain_images.data());
...
void VulkanApplication::createRenderPasses()
{
	...
	// depth pre pass // TODO: should I merge this as a
	{
		...
		VkRenderPass pass;
		if (vkCreateRenderPass(graphicsdevice, &render_pass_info, nullptr, &pass) != VK_SUCCESS)
		{
			throw std::runtime_error("failed to create depth pre-pass!");
		}
		depth_pre_pass = VulkanRaii<vk::RenderPass>(pass, renderpass_deletef);
	...
	// the render pass
	{
		...
		if (vkCreateRenderPass(graphicsdevice, &render_pass_info, nullptr, &tmp_render_pass) != VK_SUCCESS)
		{
			throw std::runtime_error("failed to create render pass!");
		}
		render_pass = VulkanRaii<vk::RenderPass>(tmp_render_pass, renderpass_deletef);
...
void VulkanApplication::createDescriptorSetLayouts()
{
	...
	// instance_descriptor_set_layout
	{
		...
		object_descriptor_set_layout = VulkanRaii<vk::DescriptorSetLayout>(
	...
	// camera_descriptor_set_layout
	{
		...
		camera_descriptor_set_layout = VulkanRaii<vk::DescriptorSetLayout>(
	...
	// light_culling_descriptor_set_layout, shared between compute pipeline and graphics pipeline
	{
		...
		light_culling_descriptor_set_layout = VulkanRaii<vk::DescriptorSetLayout>(
			device.createDescriptorSetLayout(layout_info, nullptr),
			raii_layout_deleter
			);
	}
	...
	// descriptor set layout for intermediate objects during render passes, such as z-buffer
	{
		...
		intermediate_descriptor_set_layout = VulkanRaii<vk::DescriptorSetLayout>(
			device.createDescriptorSetLayout(create_info, nullptr),
			raii_layout_deleter
			);
	}
	...
	// material_descriptror_layout // TODO: maybe I still need to do for each instance
	{
		...
		material_descriptor_set_layout = VulkanRaii<vk::DescriptorSetLayout>(
			device.createDescriptorSetLayout(create_info, nullptr),
			raii_layout_deleter
			);
	}
...
void VulkanApplication::createGraphicsPipelines()
{
	...
	{
		auto vert_shader_code = readFile(("Shaders/forwardplus_vert.spv"));
		auto frag_shader_code = readFile(("Shaders/forwardplus_frag.spv"));
		...
		auto binding_description = Utilities::getVertexBindingDesciption();
		auto attr_description = Utilities::getVertexAttributeDescriptions();
		...
		auto pipeline_layout_result = vkCreatePipelineLayout(graphicsdevice, &pipeline_layout_info, nullptr,
			&temp_layout);
		if (pipeline_layout_result != VK_SUCCESS)
		{
			throw std::runtime_error("failed to create pipeline layout!");
		}
		pipeline_layout = VulkanRaii<VkPipelineLayout>(temp_layout, raii_pipeline_layout_deleter);
		...
		auto pipeline_result = vkCreateGraphicsPipelines(graphicsdevice, VK_NULL_HANDLE, 1
			, &pipelineInfo, nullptr, &temp_pipeline);
		graphics_pipeline = VulkanRaii<VkPipeline>(temp_pipeline, raii_pipeline_deleter);
...

		{
			...
			auto depth_vert_shader_code = readFile("Shaders/depth_vert.spv");
			// auto light_culling_comp_shader_code = util::readFile(util::getContentPath("light_culling.comp.spv"));
			...
			depth_pipeline_layout = VulkanRaii<vk::PipelineLayout>(
			...
			depth_pipeline = VulkanRaii<vk::Pipeline>(
...
void VulkanApplication::createFrameBuffers()
{
	...
	// depth pass frame buffer
	{
		...
		depth_pre_pass_framebuffer = VulkanRaii<vk::Framebuffer>(
			device.createFramebuffer(framebuffer_info, nullptr),
			raii_framebuffer_deleter
			);
	}
...
void VulkanApplication::createTextureSampler()
{
	...
	texture_sampler = VulkanRaii<VkSampler>(
		sampler,
...
void VulkanApplication::createUniformBuffers()
{
	...
	{
		SceneObjectUbo ubo = {};
		ubo.model = glm::scale(glm::mat4(1.0f), glm::vec3(mScene->scale));;

		void* data;
		vkMapMemory(graphicsdevice, object_staging_buffer_memory.get(), 0, sizeof(ubo), 0, &data);
		memcpy(data, &ubo, sizeof(ubo));
		vkUnmapMemory(graphicsdevice, object_staging_buffer_memory.get());
...
void VulkanApplication::createLights()
{
	for (int i = 0; i < mScene->light_num; i++) {
		glm::vec3 color;
		do { color = { glm::linearRand(glm::vec3(0, 0, 0), glm::vec3(1, 1, 1)) }; } while (color.length() < 0.8f);
		pointlights.emplace_back(glm::linearRand(mScene->min_light_pos, mScene->max_light_pos), mScene->light_radius, color);
	}
...
void VulkanApplication::createDescriptorPool()
{
	...
	descriptor_pool = VulkanRaii<VkDescriptorPool>(
		pool,
		[device = this->device](auto& obj)
	{
...
void VulkanApplication::createSceneObjectDescriptorSet()
{
	...
	if (vkAllocateDescriptorSets(graphicsdevice, &alloc_info, &object_descriptor_set) != VK_SUCCESS)
	{
	...
	vkUpdateDescriptorSets(graphicsdevice, (uint32_t)descriptor_writes.size()
		, descriptor_writes.data(), 0, nullptr);

}

void VulkanApplication::createCameraDescriptorSet()
{
...
void VulkanApplication::createIntermediateDescriptorSet()
{
	// Create descriptor set
	{
		vk::DescriptorSetAllocateInfo alloc_info = {
...
void VulkanApplication::updateIntermediateDescriptorSet()
{
	// Write desciptor set
...
void VulkanApplication::createDepthPrePassCommandBuffer()
{
...
void VulkanApplication::createGraphicsCommandBuffers()
{
	...
	{
		vkFreeCommandBuffers(graphicsdevice, graphics_command_pool, (uint32_t)command_buffers.size(), command_buffers.data());
	}
	...
	if (vkAllocateCommandBuffers(graphicsdevice, &alloc_info, command_buffers.data()) != VK_SUCCESS)
	{
...
void VulkanApplication::createSemaphores()
{
	...
	render_finished_semaphore = VulkanRaii<vk::Semaphore>(
		device.createSemaphore(semaphore_info, nullptr),
		destroy_func
		);
	image_available_semaphore = VulkanRaii<vk::Semaphore>(
		device.createSemaphore(semaphore_info, nullptr),
		destroy_func
		);
	lightculling_completed_semaphore = VulkanRaii<vk::Semaphore>(
		device.createSemaphore(semaphore_info, nullptr),
		destroy_func
		);
	depth_prepass_finished_semaphore = VulkanRaii<vk::Semaphore>(
		device.createSemaphore(semaphore_info, nullptr),
		destroy_func
		);
}
...
void VulkanApplication::createComputePipeline()
{
	// Step 1: Create Pipeline
	{
		...
		VkPipelineLayout temp_layout;
		GResult(vkCreatePipelineLayout(graphicsdevice, &pipeline_layout_info, nullptr, &temp_layout));
		compute_pipeline_layout = VulkanRaii<VkPipelineLayout>(temp_layout, raii_pipeline_layout_deleter);

		auto light_culling_comp_shader_code = readFile("Shaders/light_culling_comp.spv");
		...
		VkPipeline temp_pipeline;
		GResult(vkCreateComputePipelines(graphicsdevice, VK_NULL_HANDLE, 1, &pipeline_create_info, nullptr, &temp_pipeline));
		compute_pipeline = VulkanRaii<VkPipeline>(temp_pipeline, raii_pipeline_deleter);
	};
}

void VulkanApplication::createLigutCullingDescriptorSet()
{
...
void VulkanApplication::createLightVisibilityBuffer()
{
...
void VulkanApplication::createLightCullingCommandBuffer()
{
...
void VulkanApplication::updateUniformBuffers(float deltatime)
{
	...
	// update camera ubo
	{
		...
		void* data;
		vkMapMemory(graphicsdevice, camera_staging_buffer_memory.get(), 0, sizeof(ubo), 0, &data);
		memcpy(data, &ubo, sizeof(ubo));
		vkUnmapMemory(graphicsdevice, camera_staging_buffer_memory.get());
	...
	// update light ubo
	{
		...
		for (int i = 0; i < light_num; i++) {
			...
			if (pointlights[i].pos.y > mScene->max_light_pos.y) {
				pointlights[i].pos.y -= (mScene->max_light_pos.y - mScene->min_light_pos.y);
			}
		}

		auto pointlights_size = sizeof(PointLight) * pointlights.size();
		void* data;
		vkMapMemory(graphicsdevice, lights_staging_buffer_memory.get(), 0, pointlight_buffer_size, 0, &data);
		memcpy(data, &light_num, sizeof(int));
		memcpy((char*)data + sizeof(glm::vec4), pointlights.data(), pointlights_size);
		vkUnmapMemory(graphicsdevice, lights_staging_buffer_memory.get());
...
void VulkanApplication::drawFrame()
{
	...
	{
		auto aquiring_result = vkAcquireNextImageKHR(graphicsdevice, swap_chain.get()
...
VulkanRaii<VkShaderModule> VulkanApplication::createShaderModule(const std::vector<char>& code)
{
	...
	auto result = vkCreateShaderModule(graphicsdevice, &create_info, nullptr, &temp_sm);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("failed to create shader module!");
	}
	return VulkanRaii<VkShaderModule>(
		temp_sm,
		[device = this->device](auto& obj)
	{
...
void VulkanApplication::CheckInput(float delta_time)
{
	mpInputManager->Update();
	if (mpInputManager->IsPressed(GLFW_KEY_A))
	{
		mCamera.position -= mCamera.rotation * vec_right * mCamera.move_speed * delta_time;
		
	}

	if (mpInputManager->IsPressed(GLFW_KEY_D))
	{
		mCamera.position += mCamera.rotation * vec_right * mCamera.move_speed * delta_time;
	}

	if (mpInputManager->IsPressed(GLFW_KEY_W))
	{
		mCamera.position += mCamera.rotation * vec_forward * mCamera.move_speed * delta_time;
	}

	if (mpInputManager->IsPressed(GLFW_KEY_S))
	{
		mCamera.position -= mCamera.rotation * vec_forward * mCamera.move_speed * delta_time;
	}

	if (mpInputManager->IsPressed(GLFW_KEY_Q))
	{
		mCamera.rotation = glm::angleAxis(mCamera.rotation_speed * 0.001f, vec_up) * mCamera.rotation; // world up
		mCamera.rotation = glm::normalize(mCamera.rotation);
	}
	if (mpInputManager->IsPressed(GLFW_KEY_E))
	{
		mCamera.rotation = glm::angleAxis(mCamera.rotation_speed * -0.001f, vec_up) * mCamera.rotation; // world up
		mCamera.rotation = glm::normalize(mCamera.rotation);
	}
