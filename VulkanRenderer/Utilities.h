...
public:
	...
	VUtility(const VulkanApplication& context);
	~VUtility() = default;
	...
	std::tuple<VulkanRaii<VkBuffer>, VulkanRaii<VkDeviceMemory>> createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags property_bits, int sharing_queue_family_index_a = -1, int sharing_queue_family_index_b = -1);
	...
	std::tuple<VulkanRaii<VkImage>, VulkanRaii<VkDeviceMemory>, VulkanRaii<VkImageView>> loadImageFromFile(std::string path);
...
private:
	...
	const VulkanApplication* context;
